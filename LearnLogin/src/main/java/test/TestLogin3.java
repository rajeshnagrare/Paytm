	...
	public static void main(String[] args) {
		...
		do {
			...
			if (userId.validateUserIdBoolean(userId.getUserId())
					&& password.validatePasswordWithRegrexBoolean(password
							.getPassword())) {
				do {
					strSystemOtp = otp.generateOTPNew();
					System.out.println("Your OTP for user " + strUserId
							+ " is :" + strSystemOtp);
					System.out
							.println("We have sent you OTP. Please enter to validate :");
					...
					if (strSystemOtp.equals(otp.getOtp())) {
						...
						System.out
								.println("Incorrect OTP. Do you want to resend OTP (y/n)");
						...
						if (resendOtp.equalsIgnoreCase("n")) {
							...
							if (intResendOtpCount > MAX_OTP_RESEND) {
								System.out
										.println("You have entered OTP incorect 3 times. No more OTP is allowed.");
								loginSuccess = true;
								break;
							} else
								intResendOtpCount++;
				...
				if (intLoginCount > MAX_LOGIN_TRY) {
					System.out
							.println("Done with 3 attempts.. Account is locked for a day.");
					break;
				} else
					intLoginCount++;
			}
